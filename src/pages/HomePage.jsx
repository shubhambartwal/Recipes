...
const HomePage = () => {
  ...
  const debouncingIngredient=useDebounce(ingredient,500)
  ...
  useEffect(() => { if(debouncingIngredient){fetchRecipes(debouncingIngredient)} }, [debouncingIngredient])
  return (
        ...
        <form>
          <label className='input shadow-md flex items-center gap-2 '><Search size={"24"} />
            <input type="text" className='text-sm md:text-md grow'
              placeholder='Mention the main ingredient of your dish' onChange={((e)=>setIngredient(e.target.value))}/>
          </label>
        </form>
